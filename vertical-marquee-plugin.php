...
<div class="wrap">
  <?php
    ...
    if($DID<>"" and $AC <> "DEL")
    {
		...
		if ( !empty($data) ) $vm_date_x = substr($data->vm_date, 0, 10);
	...
	if($vm_date_x == "")
	{
		$vm_date_x = date('Y-m-d', strtotime('+1 years'));
	}
    ?>
  ...
  <form name="form_vsm" method="post" action="<?php echo @$mainurl; ?>" onsubmit="return vsm_submit()"  >
    <table width="100%">
	  ...
	  <tr>
        <td align="left" valign="middle"><input name="vm_date" type="text" id="vm_date" size="20" value="<?php echo isset($vm_date_x) ? $vm_date_x : date('Y-m-d'); ?>" maxlength="10" /> (Format: YYYY-MM-DD)</td>
      </tr>
    ...
    <table width="100%">
      <tr>
        ...
        <td align="right">
			...
			<input name="livedemo" lang="livedemo" class="button-primary" onClick="window.open('http://www.gopiplus.com/work/2012/06/30/vertical-marquee-wordpress-plugin/');" value="Plugin Live Demo" type="button" />
			<input name="help" lang="help" class="button-primary" onClick="window.open('http://www.gopiplus.com/work/2012/06/30/vertical-marquee-wordpress-plugin/');" value="Plugin Help" type="button" />
        </td>	
      </tr>
    </table>
  </form>
        ...
        <thead>
          <tr>
            ...
            <th align="left" scope="col">Message</th>
			 <th align="left" scope="col">Expiration</th>
			...
			{ 
				...
				?>
				  <tr class="<?php if ($i&1) { echo'alternate'; } else { echo ''; }?>">
					...
					<td align="left" valign="middle"><?php echo isset($vm_date) ? $vm_date : date('Y-m-d'); ?></td>
